    ...
    studentSignupHandler: async (req, res, next) => {
        try {
            ...
            if (!email || !password || !firstName || !year || !enrollmentNo) {
                return response.badrequest(res, "Malformed input - All required fields must be provided", {});
            }

            // Validate year value
            if (!['I', 'II', 'III', 'IV'].includes(year)) {
                return response.badrequest(res, "Year must be one of: I, II, III, IV", {});
            }

            // Check if enrollment number already exists
            const existingStudent = await Student.findOne({ enrollment_no: enrollmentNo.trim() });
            if (existingStudent) {
                return response.error(res, "Enrollment number already exists", {});
            }
            ...
            student.enrollment_no = enrollmentNo.trim();
            student.year = year;
