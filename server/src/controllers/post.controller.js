...
module.exports = {
    ...
    createNewPost: async (req, res, next) => {
        try {
            ...
            if (req.user.role === roles.Student) {
                newPost.group_id = req.user.mentoredBy[0];
            }
            ...
            if (req.user.role === roles.Student) {
                const mentees = await Student.find({$and: [{
                    _id: { $ne: req.user._id }
                    },{ mentoredBy: req.user.mentoredBy[0] }
                ]});
              
                const mentor = await Mentor.findById(req.user.mentoredBy[0]);
                mentees.push(mentor);
                // generating new post notification
                await notificationController.createNotification(
                    events.POST_CREATED,
    ...
    fetchAllPosts: async (req, res, next) => {
        try {
            ...
            if (req.user.role === roles.Student) {
                const totalDocuments = await Post.countDocuments({ group_id: req.user.mentoredBy[0] });
                totalPages = Math.ceil(totalDocuments / limit);
                posts = await Post.find({ group_id: req.user.mentoredBy[0] })
                    .sort({ createdAt: -1 })
                    .skip((page - 1) * limit)
                    .limit(limit)
                    .populate("author");
    ...
    getAllPosts: async (req, res, next) => {
        try {
            const posts = await Post.find({
                $or: [
                    { author: req.user._id },
                    { group_id: { $in: req.user.mentoredBy } }
                ]
            })
            .populate("author")
            .sort({ createdAt: -1 });
    ...
    // Get posts by mentor
    getPostsByMentor: async (req, res, next) => {
        try {
            const mentees = await Student.find({ mentoredBy: { $in: [req.user._id] } });
