...
const ManageGroups = () => {
    ...
    const handleAssign = () => {
        // Show all mentees, not just unpaired ones
        setAssignMentees(students);
        setShowOverlay(true);
    ...
    const handleView = (mentorId) => {
        ...
        newMentees = students.filter(
            (student) =>
                student.mentoredBy &&
                student.mentoredBy.some(id => id.toString() === mentorId.toString())
        );
    ...
    const handleAssignMentees = () => {
        if (group.mentorId && group.studentIds.length > 0) {
            // Add the selected mentor to each student's mentoredBy array
            const updatedStudents = students.map(student => {
                if (group.studentIds.includes(student._id.toString())) {
                    // If student doesn't already have this mentor, add them
                    if (!student.mentoredBy || !student.mentoredBy.includes(group.mentorId)) {
                        return {
                            ...student,
                            mentoredBy: [...(student.mentoredBy || []), group.mentorId]
                        };
                    }
                }
                return student;
            ...
            // Update the students in the Redux store
            dispatch(adminAssignMentees({
